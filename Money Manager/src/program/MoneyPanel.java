...
public class MoneyPanel {
	static int height;
	public static List <Money> moneyList = new ArrayList<Money>();
	...
	{
		try {
			Scanner dff = new Scanner(new File("assets/testData.txt"));
			while(dff.hasNext())
			{
				int day = dff.nextInt();
	 ...
	 * 
	 * @param shortDate a string which contains a date (MM/yyyy), used for searching the incomes and expenses of a certain date
	...
	public static void addMoneyFromDatabase(String shortDate) throws Exception
	{
		...
		try {
			String sql = "select ID,to_char(mdate,'DD/MM/YYYY'),name,price from moneymanager "
					+ "where to_char(mdate,'MM/YYYY') = to_char(to_date('" + shortDate + "','MM/YYYY'),'MM/YYYY')";
			ResultSet result = Sql.st.executeQuery(sql);
	...
	{
		if(ord==1)
			moneyList.sort((m1,m2) -> {return m1.day-m2.day;});
		else
			moneyList.sort((m1,m2) -> {return m2.day-m1.day;});
	...
	public static void save()
	{
		...
		{
			sortByDate(1);
		    FileWriter wf = new FileWriter("assets/data.txt");
